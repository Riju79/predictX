...
async function main() {
  ...
  console.log('--- PHASE 7: ADD LIQUIDITY SIMULATION & SUBMISSION ---');
  ...
  let tx = new TransactionBuilder(account, { fee: '100', networkPassphrase: Networks.PUBLIC })
    .addOperation(marketContract.call('add_liquidity', user.toScVal(), nativeToScVal(testMarketId, { type: 'u64' }), nativeToScVal(amount, { type: 'i128' })))
    .setTimeout(180)
    .build();

  tx = await server.prepareTransaction(tx);
  const feePaid = Number(tx.fee) / 1e7;
  console.log('Prepared add_liquidity Tx Fee (XLM):', feePaid);

  tx.sign(keypair);
  console.log('Submitting add_liquidity transaction to Stellar Mainnet...');
  ...
  if (sendRes.status === 'PENDING') {
    ...
    console.log('🎉 ADD_LIQUIDITY TRANSACTION STATUS:', getRes.status);
    console.log('ADD_LIQUIDITY_TX_HASH:', sendRes.hash);
  }
}
